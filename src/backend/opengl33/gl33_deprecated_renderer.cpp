...
//
static GLuint screen_sized_vao;
static GLuint screen_sized_vbo;
...
static void PostProcessRenderQuad()
{
  glBindVertexArray(screen_sized_vao);
...
void GL33_InitContext(HRL_uint _width, HRL_uint _height, void* loader)
{
  ...
  //For draw screen-sized quad (to render fbo textures)
  glGenVertexArrays(1, &screen_sized_vao);
  glGenBuffers(1, &screen_sized_vbo);

  glBindVertexArray(screen_sized_vao);

  //on bind le vbo sprite
  glBindBuffer(GL_ARRAY_BUFFER, screen_sized_vbo);

  //constexpr float screen_sized_data[16] = {
  //  -1.f, 1.f, ...
  //};

  //16 car on passe 2 float et 2 coordonées UV par vertex (donc 16 en tout pour 4 vertex)
  //glBufferData(GL_ARRAY_BUFFER, 16*sizeof(float), screen_sized_data, GL_DYNAMIC_DRAW);

  //on lui bind le ebo du sprite (le meme que pour le screen sized)
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo[Sprite_Buffer]);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(sprite_indices), sprite_indices, GL_DYNAMIC_DRAW);

  //on crée ensuite les FBO qui vont rendre le jeu dans une texture : l'objet qui va nous permettre d'écrire des textures dedans
  GLuint post_fbo[2];
  GLuint post_textures[2];
  glGenFramebuffers(2, post_fbo);
  glGenTextures(2, post_textures);

  InitTextureAndBindToFBO(post_textures[0], post_fbo[0], (int)_width, (int)_height);
  InitTextureAndBindToFBO(post_textures[1], post_fbo[1], (int)_width, (int)_height);
